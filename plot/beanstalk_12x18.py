...
axs[-1, 0].set_xlabel(
    r"Increasing Instrumentation Density $\longrightarrow$", loc='left')
axs[-1, 0].set_ylabel(
    r"Different Devices $\longrightarrow$", loc='bottom')
axs[-1, -1].set_xlabel(
    r"$\longleftarrow$ Decreasing Instrumentation Density", loc='right')
axs[0, 0].set_ylabel(r"$\longleftarrow$ Different Devices", loc='top')
fig.text(
...
fig.text(
    ...
    fontsize=24, va='top')
fig.text(
    0.05, 0.05, "Detectability profile of 192 bugs in our dataset, sorted by "
    "Heisen Factor $F$. Each plot shows the detectability of a single bug "
    "across different devices (y-axis) and instrumentation densities "
    "(x-axis).", va='top')
