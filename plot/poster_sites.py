...
for i, (idx, ax) in enumerate(zip(order[start_idx:], axs.reshape(-1))):
    ...
    ax.text(
        0.98, 0.76, "{}".format(sites[idx]), ha='right', va='top',
        color='white', fontweight='bold', fontsize=7, transform=ax.transAxes)
...
fig.savefig("figures/poster_sites.pdf", bbox_inches='tight')
